...
# Configure
ARG MULTIPACK_APP_VERSION=""
ARG MULTIPACK_PROJECT_VERSION=""
RUN cmake -B build \
    -G Ninja \
    -DCMAKE_BUILD_TYPE=Release \
    -DCMAKE_CXX_STANDARD=17 \
    -DCMAKE_EXPORT_COMPILE_COMMANDS=ON \
    -DMULTIPACK_APP_VERSION="${MULTIPACK_APP_VERSION}" \
    -DMULTIPACK_PROJECT_VERSION="${MULTIPACK_PROJECT_VERSION}" \
    -DENABLE_VTK=OFF
