...
> [!NOTE]
> Palette plan files are generated by **[Multipack](https://multiscience.de/multipack-ihre-optimierungssoftware/)** software (commercial optimization software from Multiscience GmbH), which creates optimized palette plans from scratch based on input parameters (box dimensions, palette dimensions, etc.) and generates CSV files. MultipackParser reads the resulting palette plan files (`.rob` format) and provides them to the robot palletizing application.
